...
- **操作方法**：滚动鼠标滚轮即可放大/缩小（以画面中心为锚点，越滚越聚焦想看的内容）；画布左侧有一颗悬浮的圆形 **“+ / −”** 缩放按钮，点击也可逐步放大缩小；按住鼠标左键空白处可以平滑拖拽画布；点击左上方“居中视野”按钮可一键恢复全景自适应排版；顶部下拉框可秒级切换聚焦到指定章节；
- **查看详情与秒开课件**：单击任意考点节点，右侧会滑出抽屉，展示核心原理、前置基础与拓扑关联；双击节点或点击抽屉中的 **“打开对应PPT课件”** 可秒级翻开课件对应幻灯片页。
- **修改自己的图谱**：点击页面右上角 **`修改图谱`** 按钮进入编辑模式，即可增删考点节点与连线、修改节点文字；完成后点击 **`保存图谱修改`** 同步到个人云端学习空间，换设备也不丢；改乱了可点 **`恢复默认图谱`** 一键还原官方标准图谱。
...
### 小提示：偶尔看到的“暂时不可用”是什么意思？

AI 助教坚持一条规矩：**宁可如实告诉你“这次没答出来”，也绝不编一段假答案或假页码糊弄你**。所以如果偶尔看到下面这些提示，属于正常保护，不是网站坏了：

| 提示 | 含义 | 你只需要 |
| :--- | :--- | :--- |
| “出题服务本次未返回有效题目（已自动重试一次）” | 出题 AI 这次没出合格题目，系统拒绝拿差题充数 | 稍等片刻再试；**学情诊断中直接回复“下一题”**就能补发新题，已答的题不会丢 |
| “这道题没有携带可核验的标准答案，已停止判分” | 题目数据不完整，系统拒绝乱判分 | 回复“下一题”换一道题 |
| “课程助教服务暂时无法连接” | 网络波动，平台不会离线编造答案 | 检查网络后重新发送问题 |
