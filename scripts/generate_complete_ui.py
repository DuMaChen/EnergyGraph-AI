...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .upload-status-msg.success { background: #dcfce7; color: #166534; border: 1px solid #bbf7d0; }
    .upload-status-msg.error { background: #fee2e2; color: #991b1b; border: 1px solid #fecaca; }
    ...
    .zhs-progress-badge { text-align: right; }
    .zhs-progress-num { font-size: 2.2rem; font-weight: 800; line-height: 1; }
    .zhs-progress-label { font-size: .84rem; opacity: 0.88; margin-top: 4px; }
    ...
    .zhs-filter-tabs { display: flex; gap: 18px; }
    .zhs-filter-tab {
      cursor: pointer;
      font-size: .92rem;
      font-weight: 600;
    ...
    .zhs-resource-row:hover { background: #f8fafc; }
    .zhs-res-name {
      display: flex;
      align-items: center;
      gap: 12px;
    ...
    .reader-doc-title { font-size: .95rem; font-weight: 650; color: #f1f5f9; }
    .reader-top-right { display: flex; align-items: center; gap: 14px; }
    ...
    .reader-side-drawer.collapsed { margin-right: -320px; }
    ...
    .reader-drawer-tree { flex: 1; overflow-y: auto; padding: 10px 0; }
...
<body>
  <div class="app-container">
    ...
    <main id="mainTabContent">
      ...
      <section id="sectionStudyResources" class="tab-pane">
        ...
        <!-- 教师专属：上传课件 PDF 操作面板 -->
        <div class="teacher-upload-card" id="teacherUploadCard" style="display:none;">
          <div class="panel-header" style="margin-bottom:12px; padding-bottom:8px;">
            <div>
              <h3 style="margin:0; font-size:1.1rem; color:var(--primary); font-weight:750;">教师课件上传与知识库发布</h3>
              <div style="font-size:.84rem; color:var(--muted)">上传 PPT 转换的 PDF 教材或拓展讲义（单文件最大 30MB），系统将自动收录入智慧树资源列表与 AI 知识图谱</div>
            </div>
            <span class="drawer-tag">教师权限专享</span>
          </div>

          <form id="pdfUploadForm" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)) 1.2fr; gap:14px; align-items:end;">
            <div>
              <label style="font-weight:650; font-size:.86rem; color:var(--ink); display:block; margin-bottom:4px;">所属章节分类</label>
              <select id="uploadChapterSelect" class="form-control">
                <option value="1">第1章 概述</option>
                <option value="2">第2章 电力系统与储能技术的应用</option>
                <option value="3" selected>第3章 电力储能系统的组成及工作原理</option>
                <option value="4">第4章 电力储能系统的规划配置</option>
                <option value="5">第5章 电力储能系统的接入与运行维护</option>
                <option value="6">第6章 电力储能系统的性能检测与评估</option>
              </select>
            </div>

            <div>
              <label style="font-weight:650; font-size:.86rem; color:var(--ink); display:block; margin-bottom:4px;">课件显示名称</label>
              <input type="text" id="uploadTitleInput" class="form-control" placeholder="例如：3.6 储能系统安全防护与热失控预警.pdf" />
            </div>

            <div>
              <label style="font-weight:650; font-size:.86rem; color:var(--ink); display:block; margin-bottom:4px;">选择 PDF 课件文件</label>
              <div style="display:flex; gap:8px;">
                <input type="file" id="pdfFileInput" accept=".pdf,application/pdf" style="display:none;" />
                <button type="button" id="btnBrowsePdfFile" class="btn btn-secondary" style="flex:1; white-space:nowrap;">
                  浏览本地 PDF
                </button>
                <button type="submit" id="btnSubmitPdfUpload" class="btn btn-primary" style="white-space:nowrap;" disabled>
                  上传并收录
                </button>
              </div>
            </div>
          ...
          <div id="uploadSelectedFileInfo" style="margin-top:8px; font-size:.85rem; color:var(--ink-secondary); display:none;">
            <span>已选文件：<strong id="selectedFileNameText"></strong> (<span id="selectedFileSizeText"></span>)</span>
          </div>

          <div id="uploadStatusMsgBox" class="upload-status-msg"></div>
        </div>
          ...
          <div>
            <div class="zhs-filter-bar">
              <div class="zhs-filter-tabs">
                <span class="zhs-filter-tab active" data-filter="all" id="tabFilterAll">全部 (21)</span>
                <span class="zhs-filter-tab" data-filter="completed" id="tabFilterCompleted">已完成 (21)</span>
    ...
    <div class="reader-body-layout">
      ...
      <div class="reader-side-drawer" id="readerSideDrawer">
        <div class="reader-drawer-header">
          <span id="drawerTotalCountText">学习资源包 (21)</span>
  ...
  <script>
    ...
    function getAllResources() {
      return COURSE_CHAPTERS.flatMap(c => c.resources);
    }
    ...
    let activeReadingResource = COURSE_CHAPTERS[0].resources[0];
    ...
    function renderStudyResourcePackages() {
      ...
      document.querySelector('#tabFilterAll').textContent = `全部 (${allRes.length})`;
      document.querySelector('#tabFilterCompleted').textContent = `已完成 (${allRes.length})`;
      document.querySelector('#drawerTotalCountText').textContent = `学习资源包 (${allRes.length})`;
    ...
    document.querySelector('#btnQuickResume').addEventListener('click', () => {
      const allRes = getAllResources();
      openZhihuishuReader(allRes[0], 1);
    });
    ...
    const browseBtn = document.querySelector('#btnBrowsePdfFile');
    const submitBtn = document.querySelector('#btnSubmitPdfUpload');
    const fileInfoBox = document.querySelector('#uploadSelectedFileInfo');
    const fileNameText = document.querySelector('#selectedFileNameText');
    const fileSizeText = document.querySelector('#selectedFileSizeText');
    const statusMsgBox = document.querySelector('#uploadStatusMsgBox');
    const uploadTitleInput = document.querySelector('#uploadTitleInput');
    const uploadChapterSelect = document.querySelector('#uploadChapterSelect');
    ...
    function handleSelectedPdf(file) {
      if (!file.name.toLowerCase().endsWith('.pdf')) {
        showUploadStatus('请选择以 .pdf 结尾的课件文件！', 'error');
        return;
      }
      if (file.size > 30 * 1024 * 1024) {
        showUploadStatus('文件超过 30MB 限制，请先压缩后再上传！', 'error');
        return;
      }
      ...
      if (!uploadTitleInput.value.trim()) {
        uploadTitleInput.value = file.name;
      }

      submitBtn.disabled = false;
      showUploadStatus('已就绪，点击“上传并收录”即可发布。', 'success');
    }

    function showUploadStatus(msg, type) {
      statusMsgBox.textContent = msg;
      statusMsgBox.className = `upload-status-msg ${type}`;
      statusMsgBox.style.display = 'block';
    }

    document.querySelector('#pdfUploadForm').addEventListener('submit', async (e) => {
      e.preventDefault();
      if (!selectedPdfFile) return;

      const ch = Number(uploadChapterSelect.value) || 1;
      const title = uploadTitleInput.value.trim() || selectedPdfFile.name;

      submitBtn.disabled = true;
      submitBtn.textContent = '正在上传中...';
      showUploadStatus('正在上传 PDF 并提取章节考点...', 'success');

      try {
        const formData = new FormData();
        formData.append('chapter_id', ch);
        ...
        if (!res.ok || data.status === 'error') {
          throw new Error(data.error?.message || '上传处理失败');
        }

        const uploadedItem = {
          id: data.data.resource?.id || `custom-${Date.now()}`,
          title: data.data.title || title,
          file: data.data.file,
          pages: data.data.pages || 15,
          completed: true,
          isCustomUpload: true,
          pdfUrl: data.data.pdf_url
        };

        const targetChapter = COURSE_CHAPTERS.find(c => c.id === ch);
        if (targetChapter) {
          targetChapter.resources.push(uploadedItem);
        }

        showUploadStatus(`课件《${title}》上传成功并已收录至第 ${ch} 章！`, 'success');
        renderStudyResourcePackages();
        ...
        submitBtn.textContent = '上传并收录';
        submitBtn.disabled = true;

        setTimeout(() => {
          openZhihuishuReader(uploadedItem, 1);
        }, 800);
      } catch (err) {
    ...
    function openZhihuishuReader(resource, page = 1) {
      ...
      const pdfBase = resource.pdfUrl ? resource.pdfUrl : `/agent/pdf/${encodeURIComponent(resource.file)}`;
      const pdfUrl = `${pdfBase}#page=${currentReaderPage}`;
      iframe.src = pdfUrl;
      downloadBtn.href = pdfBase;
    ...
    function renderReaderSideDrawerTree() {
      ...
      COURSE_CHAPTERS.forEach(ch => {
        ...
        ch.resources.forEach(r => {
          ...
          item.innerHTML = `
            <span>${r.title}</span>
            <span style="font-size:10px; color:var(--zhs-green);">已学完</span>
          `;
          item.onclick = () => {
    ...
    function updateReaderPage() {
      ...
      const pdfBase = activeReadingResource.pdfUrl ? activeReadingResource.pdfUrl : `/agent/pdf/${encodeURIComponent(activeReadingResource.file)}`;
      iframe.src = `${pdfBase}#page=${currentReaderPage}`;
    }
    ...
    function showNodeDrawer(node) {
      ...
      btnPdf.onclick = () => {
        const allRes = getAllResources();
        const matchingRes = allRes.find(r => r.id === node.id) || allRes[0];
...
print("Saved complete agent-ui/index.html with teacher PDF upload")
