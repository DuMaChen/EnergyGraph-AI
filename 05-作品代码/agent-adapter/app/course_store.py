...
class CourseStore:
    ...
    def save_mindmap(self, workspace_key: str, data: dict[str, Any], user_uid: str = "teacher", role: str = "teacher") -> dict[str, Any]:
        data_json = json.dumps(data, ensure_ascii=False)
        now = datetime.now(timezone.utc).isoformat()
        with self._lock, self.connect() as db:
            db.execute(
                """
                INSERT INTO mindmap_snapshots (workspace_key, user_uid, role, data_json, updated_at)
                VALUES (?, ?, ?, ?, ?)
                ...
                """,
                (workspace_key, user_uid, role, data_json, now)
            )
            db.commit()
        return {"workspace_key": workspace_key, "updated_at": now}

    def get_mindmap(self, workspace_key: str) -> dict[str, Any] | None:
        with self._lock, self.connect() as db:
            row = db.execute(
                "SELECT workspace_key, user_uid, role, data_json, updated_at FROM mindmap_snapshots WHERE workspace_key = ?",
                (workspace_key,)
            ).fetchone()
            if not row:
                return None
    ...
    def reset_mindmap(self, workspace_key: str) -> bool:
        with self._lock, self.connect() as db:
            cursor = db.execute(
                "DELETE FROM mindmap_snapshots WHERE workspace_key = ?",
                (workspace_key,)
            )
            db.commit()
