...
也可以使用带前置门禁的一键入口。它会先确认 Mock 模式关闭和 P0 服务运行，
再执行上述真实冒烟；缺少配置时不会发起上游请求：
...
可以先用组装脚本把已审核的非敏感报告复制到提交目录，再人工检查文件内容：

```bash
bash scripts/assemble_submission_materials.sh
```

作品代码附件可单独生成。脚本使用显式源文件清单，并在生成后检查压缩包
成员，排除服务器 `.env`、密钥、备份、课程原始文件和缓存：
