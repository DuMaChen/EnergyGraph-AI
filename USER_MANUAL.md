...
2. **沉浸式课件阅读器**：
   - 点击任一课件可直接打开内置高清 PDF 阅读器，支持全屏、缩放与页面任意跳转。
3. **划词定向追问（特色交互）**：
   - 在课件文本中**用鼠标选定任意专业词汇或段落**（如“平准化度电成本 LCOS”、“变流器下垂控制”），系统会自动弹出快捷操作气泡 **“向助教提问”**。
   - 点击后将带入选中文本与课件页码上下文直达 AI 助教，由大模型针对该段落进行深度启发式讲解。
...
#### ③ 智能出题与交互批改
- 发送 **“给我出一道第3章的考题”** 或点击 **“考点自测”**。
- 助教会生成包含情景题干的单选题，并在对话流中渲染为**可交互的选项卡片（A / B / C / D）**。
- 学生直接点击选项卡片或输入 `我选B`，助教即刻进行批改判分并给出深度原理解析。

#### ④ 储能工程情景演练（实训角色扮演）
- 发送 **“开启情景演练”** 或 **“模拟储能电站故障排查”**。
- 助教将切换为 **“储能电站资深运维工程师”** 或 **“电网调度员”** 角色，设定真实工况（如“某 100MW 储能电站集装箱电池温升过高且绝缘阻抗告警”），引导学生一步步排查故障原因并制定控制策略。
- 输入 **“退出演练”** 即可随时恢复为标准助教模式。
...
| 模块 / 特色 | 技术实现原理 | 给师生带来的价值 |
| :--- | :--- | :--- |
| 🚀 **极速学情诊断** | 前端全量做题元数据结构化直传 + 轻量收敛检索 | 0.8s 极速响应，彻底消除模型脱离实际瞎编乱造，精准对症下药 |
| 📐 **公式渲染引擎** | 深度栈平衡扫描器 + 本地 KaTeX 双重保障 | 复杂积分、多重括号、单字母变量公式 100% 优雅学术级排版 |
| 🧭 **确定性思维导图** | 放射状星形聚类几何算法 | 彻底取消小球晃动与物理漂移，全景知识拓扑一目了然 |
| 📖 **16ms 课件穿透** | 全局事件总线 + PDF 渲染器页码定位 | 从助教回答或思维导图一键直跳课件对应页，学习不中断 |
| 🛡️ **安全双重隔离** | FastAPI 适配层 + Moodle RBAC 角色鉴权 | 学生无法绕过权限越权改分或窥探题库，保障考核公平公正 |
