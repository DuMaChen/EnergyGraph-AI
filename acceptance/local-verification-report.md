...
| 提交材料完整性 | `bash scripts/check_submission_materials.sh` | 按预期返回非零并列出 3 项真实缺口：正式 Demo 地址、实际录屏、真实人工验收汇总；同时校验 HTTPS 域名、视频类型、人工汇总字段、代码附件 SHA-256 和压缩包成员，七个提交目录和基线报告均存在 |
| 真实 Workflow 前置门禁 | `bash scripts/real_workflow_preflight.sh deploy/.env` | 按预期安全失败：检测到 Workflow 凭据缺失后不发起上游请求、不输出密钥；凭据到位后才会检查 Mock 模式、P0 服务和真实五次冒烟 |
| 提交材料组装 | `bash scripts/assemble_submission_materials.sh` | 通过；只复制方案、合规、验收和部署说明，未复制 `.env`、密钥、课程原始文件、备份或服务器数据 |
| 作品代码打包 | `bash scripts/package_submission_code.sh` | 通过；生成 134 个源文件成员和 SHA-256，压缩包未包含 `deploy/.env`、密钥类文件、课程原始 PDF 或缓存 |
