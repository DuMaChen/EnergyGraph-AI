...
class TestWorkflowE2ELive(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._prev_mock_wf = os.environ.get("MOCK_WORKFLOW_MODE")
    ...
    def tearDownClass(cls):
        ...
        if cls._prev_mock_wf is not None:
            os.environ["MOCK_WORKFLOW_MODE"] = cls._prev_mock_wf
        else:
            os.environ.pop("MOCK_WORKFLOW_MODE", None)
    ...
    def test_08_interactive_diagnosis_e2e_full_flow(self):
        """Scenario 8: Multi-step interactive diagnosis from Q1 -> Q2 -> Report Generation"""
        ...
        self.assertEqual(resp1.status_code, 200)
        self.assertIn("互动学情诊断测评", resp1.text)
        ...
        self.assertEqual(resp2.status_code, 200)
        self.assertIn("event: quiz_graded", resp2.text)
        self.assertIn("互动学情诊断测评（第 2 题", resp2.text)
        self.assertIn("event: quiz_meta", resp2.text)

        # 3. Request report generation
        resp3 = client.post(
            "/api/course-agent/chat",
            headers=HEADERS,
            json={"question": "生成诊断报告", "session_id": session_id, "mode": "qa"}
        )
        ...
        self.assertTrue("学情诊断综合报告" in full_text3 or "互动学情诊断" in full_text3)
        self.assertIn('"scene_mode": 0', resp3.text)
        self.assertIn('"reason": "interactive_diagnosis_completed"', resp3.text)
        print("Scenario 8 (Interactive Diagnosis Flow & Report): PASS")
